...
**2026-08-13 잔여분 처리(완전 해결 아님 — 아래 계속 미해결 항목 참고)**: [ADR-0016](./docs/adr/0016-accessibility-followup.md)으로 위 미해결 항목 중 첫 번째(캔버스 그래프의 키보드 접근성)를 처리했다. Cytoscape canvas 자체를 접근 가능하게 만들 수 없어서, `ImpactGraph.tsx`에 같은 subgraph 데이터를 키보드로 순회 가능한 목록으로도 보여주는 "그래프/목록" 토글을 추가했다(새 API 호출 없음, 노드/엣지 선택 콜백은 캔버스와 동일하게 재사용). 이 작업의 axe-core 재검증 과정에서 **2026-08-12 감사 이후 신규 추가된 화면**(구조 탭 `StructureTree.tsx`, ADR-0013)에서 실제 위반을 하나 발견해 즉시 고쳤다 — 펼침 버튼이 행 전체의 `role="button"` 안에 중첩돼 있던 `nested-interactive`(serious) 위반으로, 행 전체가 아니라 이름 텍스트만 클릭 가능하게 하고 펼침 버튼과 형제 관계로 분리했다. 재검증 후 구조 탭·검토 탭(사각지대 섹션 포함)·ImpactGraph 그래프/목록 뷰 전부 axe-core WCAG 2 A/AA 위반 0건, 목록 뷰의 노드/엣지 행 모두 Tab+Enter만으로 실제 동작(엔티티 이동/Evidence 패널 오픈)까지 확인했다.

**여전히 미해결 (상태를 [해결됨]으로 올리지 않는 이유)**: **스크린리더 실기 테스트(VoiceOver/NVDA)는 여전히 하지 않았다** — 이번엔 "설계 과제라 미룬다"가 아니라 이 작업이 실행되는 헤드리스 CLI 환경에는 실제 GUI 스크린리더를 구동할 방법이 없다는 환경적 한계다. axe-core는 WCAG 규칙 위반을 정적으로 잡아줄 뿐 "스크린리더 사용자가 실제로 이 화면을 어떻게 경험하는가"를 대체하지 못한다 — 이 갭을 감추지 않고 그대로 남긴다. 실제 데스크톱 환경에서 사람이 확인하기 전까지 [부분 해결] 상태를 유지한다. IMPLEMENTATION_REPORT.md §24 참고.
...
| 2026-08-13 | 1.1 | 5.20 잔여분(캔버스 그래프 키보드 접근성)을 처리 — [ADR-0016](./docs/adr/0016-accessibility-followup.md). Cytoscape canvas는 접근 가능하게 만들 수 없어 `ImpactGraph.tsx`에 같은 subgraph 데이터를 키보드로 순회 가능한 "목록" 뷰 토글을 추가했다(새 API 없음). axe-core 재검증 과정에서 2026-08-12 감사 이후 신규 추가된 구조 탭(`StructureTree.tsx`, ADR-0013)에서 실제 `nested-interactive`(serious) 위반을 발견해 즉시 고쳤다 — 이번 작업 자체가 발견한 회귀. 구조/검토/ImpactGraph 그래프·목록 뷰 전부 axe-core WCAG 2 A/AA 위반 0건, 목록 뷰 키보드 시나리오(Tab+Enter로 노드 이동·Evidence 패널 오픈)까지 실측 확인했다. 스크린리더 실기(VoiceOver/NVDA) 테스트는 헤드리스 CLI 환경의 한계로 여전히 못했다는 사실을 감추지 않고 명시해, 5.20은 [부분 해결] 상태를 유지한다. 이로써 이 문서가 제안한 모든 항목(P0~P2)이 [해결됨] 또는 [부분 해결](의도적으로 남긴 갭이 명시된 상태)로 정리됐다. IMPLEMENTATION_REPORT.md §24 참고 |
