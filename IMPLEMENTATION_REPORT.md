...
## 18. 부록 — 프로젝트 카탈로그 포지셔닝 + 보안 로드맵 (BENCHMARK.md 5.9·5.12, 2026-08-12)

골든 fixture 회귀 하네스(§17) 완료 후 BENCHMARK.md에 남아있던 P0 3개(5.4, 5.9, 5.12) 중 5.4를 먼저 처리했고, 이어서 사용자가 나머지 둘 — 5.9(프로젝트 카탈로그 포지셔닝)와 5.12(보안/인증 로드맵) — 을 함께 진행해달라고 요청했다. 둘 다 코드 변경보다 "방향을 정하고 문서로 명문화"하는 성격이 커서, 이 프로젝트가 이런 순간에 쓰는 관례대로 각각 ADR로 기록했다.

### 5.9 — ADR-0009: 카탈로그는 코드 관계 그래프의 보조 기능

경쟁 벤치마킹(BENCHMARK.md 3.6)이 지적한 것: Phase 2가 만든 기능(Project 등록/검색·기술 스택 태깅·유사 프로젝트 탐색)은 Backstage류 "서비스 카탈로그" 제품군과 기능적으로 겹치는데, "카탈로그를 더 키울지 보조 기능으로 얕게 둘지" 결정 자체가 로드맵에 없었다.

BENCHMARK.md §4가 이미 정확한 표현을 갖고 있었다 — "카탈로그 제품에 코드 그래프를 얹은 것이 아니라 코드 관계 그래프에 최소한의 카탈로그 레이어가 자연스럽게 얹힌 것". [ADR-0009](./docs/adr/0009-catalog-positioning.md)는 이 표현을 공식 결정으로 승격했을 뿐 새 제약을 추가하지 않았다 — ADR-0006(유사도 = 순수 태그 교집합, Vector Search 아님)이 이미 이 방향으로 설계되어 있었고, claude-do.md의 "다중 Project 지식 그래프 확장 금지"와도 직결된다. README.md 프로젝트 목적 문단에 Backstage 대비 차별점(수동 YAML 대신 `package.json` 자동 감지, 카탈로그가 관계 그래프와 조회 계층을 공유) 두 문장을 추가했고, PRD.md는 "1.1 제품 정의" 자체를 다시 쓰지 않고 상태 문구에 ADR-0009로의 포인터만 추가했다 — Phase 2 확장을 PRD 본문이 아니라 별도 ADR로 다루는 기존 패턴(ADR-0004)을 그대로 따른 것이다.

### 5.12 — ADR-0010: 로컬 단일 사용자 전제의 유효기간 + 옵트인 API Key

HTTP API/MCP 모두 인증이 없다는 것 자체는 §10에 이미 알려진 제한사항으로 적혀 있었다. 문제는 두 가지가 결정되지 않은 채였다는 것: "이 전제가 언제까지 유효한가"와 "그동안 최소한의 보호장치는 넣을 것인가"(§11-5의 기존 권고).

[ADR-0010](./docs/adr/0010-security-roadmap.md)의 결정: (1) 원격 다중 사용자 배포가 실제로 요청되기 전까지 로컬 단일 사용자 전제를 유지한다, (2) 그동안에도 파괴적/변경 오퍼레이션에는 옵트인 API key를 추가한다.

**구현**:

- `packages/api/src/errors.ts` — `UNAUTHORIZED`(401) 에러 코드 추가.
- `packages/api/src/app.ts` — `AppContext.apiKey`가 설정된 경우에만 `GET`/`HEAD`/`OPTIONS`를 제외한 모든 요청에 `x-api-key` 헤더 일치를 요구하는 미들웨어(`requireApiKey`)를 `router`(모든 `/api/v1` 라우트가 여기 마운트됨) 최상단에 등록. `GET /health`는 `router`가 아니라 `app`에 직접 등록되어 있어 이 미들웨어의 영향을 전혀 받지 않는다 — Docker healthcheck가 키 없이 계속 동작해야 하기 때문. 미설정 시(기본값)에는 미들웨어 자체가 등록되지 않아 기존 동작과 100% 동일하다.
- `packages/api/src/index.ts` — `--api-key` CLI 플래그 / `CONTEXTSOURCE_API_KEY` 환경변수로 노출.
- **범위를 GET에는 적용하지 않은 이유**: 실제 위협은 "조회당함"이 아니라 "삭제·변조당함"이다 — 소스 코드 자체가 로컬에 있고 Query 결과도 그 코드에서 파생된 값이라 조회 노출의 위험이 삭제보다 낮다. Web UI가 매 조회마다 키를 주입해야 하는 마찰도 피한다.
- **Web UI에는 키를 심지 않았다** — 브라우저에 배포되는 정적 JS 번들의 값은 페이지에 접근할 수 있는 누구에게나 보이므로 진짜 비밀이 될 수 없다(security theater). 그래서 API key를 켜면 Web UI의 쓰기 동작(프로젝트 등록/삭제, 기술 스택 편집, 분석 실행)도 함께 401로 막힌다 — 버그가 아니라 이 옵션의 위협 모델("API 포트에 직접 접근하는 임의의 네트워크 클라이언트를 막는다")이 "Web UI만 예외로 신뢰한다"를 포함하지 않기 때문이다. README.md·docker-compose.yml·API.md 1.4에 이 상호작용을 명시했다.
- `packages/api/test/api-key.test.ts` — 실제 HTTP 서버로 7개 테스트: `apiKey` 설정 시 GET은 계속 열려있음, `GET /health`는 항상 열려있음, `POST`/`PATCH`/`DELETE`는 키 없음·틀림 시 401, 맞을 때만 통과. 기존 `app.test.ts` 등은 `apiKey`를 설정하지 않으므로 전혀 영향받지 않음을 전체 스위트 194개 통과로 확인.
- **openapi.yaml 부수 효과**: `ApiKeyAuth` 시큐리티 스킴을 루트 레벨에 `security: [{}, {ApiKeyAuth: []}]`(인증 없음 또는 API key 둘 다 허용 — "설정 안 하면 열려있고 설정하면 요구된다"는 실제 동작 그대로)로 추가했더니, 이전까지 모든 endpoint에 걸쳐 있던 `security-defined` redocly lint 에러 23건이 전부 해소됐다(`redocly lint` 결과 23 errors → 0 errors). 의도한 작업은 아니었지만 스펙의 실제 결함을 하나 더 닫은 셈이다.
