...
export default function App() {
  ...
  async function triggerRun(mode: 'full' | 'incremental') {
    ...
    setRunError(null);
    try {
      const { status, body } = await api.triggerRun(mode);
      if (status >= 400) {
      ...
      <main className="layout">
        {tab === 'overview' && (
          <div className="content" style={{ width: '100%' }}>
            <Overview refreshKey={refreshKey} onSelectEntity={goToEntity} onGoToReview={() => setTab('review')} />
          </div>
        )}
        {tab === 'explore' && (
          <Explore selectedEntityId={selectedEntityId} onSelectEntity={setSelectedEntityId} />
        )}
        {tab === 'review' && (
          <div className="content" style={{ width: '100%' }}>
            <Review refreshKey={refreshKey} onSelectEntity={goToEntity} />
          </div>
        )}
        {tab === 'history' && (
          <div className="content" style={{ width: '100%' }}>
